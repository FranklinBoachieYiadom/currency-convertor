...
const fallback: Record<string, number> = { USD: 1, GHS: 11.4, KZT: 453.99, GBP: .74, EUR: .92, JPY: 149.6, CAD: 1.36, AUD: 1.52 }
...
function App() {
  ...
  const format = (value: number) => new Intl.NumberFormat(undefined, { maximumFractionDigits: precision === 'auto' ? 2 : precision, minimumFractionDigits: precision === 'auto' ? 0 : precision }).format(value)
  const baseRate = rates[base] || 1
  const value = (code: string) => amount * ((rates[code] || fallback[code] || 1) / baseRate)
  const inputValue = (number: number) => String(Number(number.toFixed(precision === 'auto' ? 4 : precision)))
  const currency = (code: string) => currencies.find(c => c.code === code) || currencies[0]
  const selectCode = (index: number) => { const next = window.prompt('Enter a currency code: USD, GHS, KZT, GBP, EUR, JPY, CAD, or AUD'); if (!next) return; const code = next.toUpperCase(); if (!currencies.some(c => c.code === code)) return alert('That currency is not available in this demo.'); const list = [...codes]; list[index] = code; setCodes(list); if (index === 0) setBase(code) }
  ...
  const changeValue = (code: string, raw: string) => { const next = Number(raw); if (!Number.isFinite(next)) return; const rate = (rates[code] || fallback[code] || 1) / baseRate; setAmount(next / rate) }
  ...
  return <main className="page"><section className="converter">
    ...
    <div className="rows">{rows.map((item, index) => <article className={'currency-row ' + (item.code === base ? 'base' : '')} key={item.code} draggable onDragStart={() => setDragIndex(index)} onDragOver={e => e.preventDefault()} onDrop={() => moveCurrency(index)} onDragEnd={() => setDragIndex(null)}>
      ...
      <input aria-label={`${item.code} amount`} type="number" value={inputValue(value(item.code))} onChange={e => changeValue(item.code, e.target.value)} />
